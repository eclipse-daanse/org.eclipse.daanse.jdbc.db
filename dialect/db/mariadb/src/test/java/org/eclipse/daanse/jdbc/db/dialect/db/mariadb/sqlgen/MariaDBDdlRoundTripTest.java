 ...
 */
package org.eclipse.daanse.jdbc.db.dialect.db.mariadb.sqlgen;
...
class MariaDBDdlRoundTripTest {

    @Container
    @SuppressWarnings("resource")
    static final MariaDBContainer<?> CONTAINER = new MariaDBContainer<>("mariadb:11.3")
            // schema == database in MariaDB; provision under the test's name.
    ...
    void full_round_trip_with_per_step_database_service_verification() throws Exception {
        ...
        // CREATE
        // CREATE SCHEMA IF NOT EXISTS — harmless because container provisions RT_TEST.
        executeAndVerify(dialect.ddlGenerator().createSchema(SCHEMA.name(), true), info -> {
            /* schema may show up in catalogs() on MariaDB, not schemas() */ });
        ...
        // MariaDB note: cannot reference target table in a subquery — use JOIN syntax
        // instead.
        ...
        // Don't DROP SCHEMA — the container's user only owns RT_TEST and we'd
        // need root privileges to recreate it; tearDown re-uses the schema anyway.
    }
