 ...
 */
package org.eclipse.daanse.jdbc.db.dialect.db.mssqlserver.sqlgen;
...
class MsSqlServerDdlRoundTripTest {
    ...
    void setUp() throws Exception {
        ...
        this.dataSource = ds;
        this.dialect = new MicrosoftSqlServerDialect();
    ...
    void full_round_trip_with_per_step_database_service_verification() throws Exception {
        ...
        executeAndVerify(dialect.ddlGenerator().createSchema(SCHEMA.name(), true), info -> {
            /* SQL Server: schema is a namespace inside the master DB */ });
        executeAndVerify(dialect.ddlGenerator().createTable(CUSTOMERS, custCols, custPk, true), info -> {
            /* loader may not surface tables on first call — drop side-effect verifies */ });
        ...
        // Cross-table UPDATE — CUSTOMERS.NAME ← MAX(ORDERS.NOTE) per customer.
        // SQL Server: UPDATE … SET … FROM … JOIN — T-SQL flavour.
        String qC = dialect.quoteIdentifier(SCHEMA.name(), CUSTOMERS.name());
        String qO = dialect.quoteIdentifier(SCHEMA.name(), ORDERS.name());
        try (PreparedStatement ps = connection.prepareStatement("UPDATE C SET " + dialect.quoteIdentifier("NAME")
                + " = O.NEW_NAME" + " FROM " + qC + " AS C" + " JOIN (" + "  SELECT "
                ...
                + " ON O." + dialect.quoteIdentifier("CUSTOMER_ID") + " = C." + dialect.quoteIdentifier("ID"))) {
        ...
        executeAndVerify(dialect.ddlGenerator().dropSchema(SCHEMA.name(), true, true), info -> {
        });
    }
