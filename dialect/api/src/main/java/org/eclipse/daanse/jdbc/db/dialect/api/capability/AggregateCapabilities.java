...
/**
 * Describes the aggregate function capabilities of a SQL dialect.
 * <p>
 * This record groups all aggregate-related capability checks into a single,
 * cohesive object, making it easier to pass around and test for dialect capabilities.
 *
 * @param countDistinct whether the dialect supports COUNT(DISTINCT column)
 * @param multipleCountDistinct whether the dialect supports multiple COUNT(DISTINCT) in the same query
 * @param compoundCountDistinct whether the dialect supports COUNT(DISTINCT col1, col2)
 * @param countDistinctWithOtherAggs whether COUNT(DISTINCT) can be used with other aggregates in the same query
 * @param innerDistinct whether the dialect allows DISTINCT in inner/subqueries
 * @param multipleDistinctSqlMeasures whether the dialect supports multiple distinct SQL measures
 * @param groupingSets whether the dialect supports GROUPING SETS
 * @param groupByExpressions whether the dialect supports expressions in GROUP BY
 * @param selectNotInGroupBy whether the dialect allows SELECT columns not in GROUP BY (MySQL-style)
 */
public record AggregateCapabilities(
    ...
    boolean groupByExpressions,
    boolean selectNotInGroupBy
) {

    /**
     * Creates capabilities with all features enabled.
     * Useful as a starting point for most modern databases.
     *
    ...
    /**
     * Creates capabilities with minimal features - only basic COUNT(DISTINCT).
    ...
    /**
     * Creates capabilities with no aggregate features.
     *
